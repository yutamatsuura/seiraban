...
def parse_gogyou_kantei_final(raw_text):
    """五行による鑑定の最終修正版解析 - 順次処理による完全分離"""
    results = []
    ...
    gogyou_content = gogyou_content_original = gogyou_section_match.group(1).strip()
    ...
    if jinko_match:
        ...
        # 処理済み部分を除去
        gogyou_content = gogyou_content[jinko_match.end():].strip()
...
# 解析実行
results = parse_gogyou_kantei_final(test_text)
